...
data = pd.read_excel("DLdata1.xlsx")
...
lr = 0.0002   # Learning rate (eta)
n1 = 8     # number of neurons (Hidden layer)
n2 = 1     # number of neurons (Out-put layer)
layer = [1,1]    # topology including input
epoch = 70 # Number of epochs
...
w1_init = np.random.uniform(-1,1,(input_size,n1))  # Random numbers in the range of -1 to 1 with uniform distribution
w2_init = np.random.uniform(-1,1,(n1,n2))

w1 = w1_init  # Weights of hidden layer
w2 = w2_init  # Weights of out-put layer
...
for i in range(epoch):  # Iteration over epochs    i: number of epoch
    ...
    for j in range(data_train_size):  # Iteration over rows   j: number of row
        ...
        for k in range(W.__len__()):
            if k == 0:
                ...
                temp = np.reshape(W[layersLength-k],(W[layersLength-k].shape[1],W[layersLength-k].shape[0]))
        ...
        for k in range(gradTemp.__len__()):
            if k == 0:
                gradTemp[k] = np.dot(input_data[j,:],gradTemp[k])
                gradTemp[k] = np.reshape(gradTemp[k], (1, gradTemp[k].shape[0]))
    ...
    print("weight : " , W[0][0][0] , "baias : " , baias)
...
i = input_data
feedForwardOut = W[0][0][0]*i+baias
...
axes[1,1].plot(i,targets_data,'bo')
axes[1,1].plot(i , feedForwardOut , 'r')
