...
def feedForward(WL,WU,C,SIGMA,input_data,layer):
    outList = []
    for j in range(np.shape(input_data)[0]):
        ...
        out2U = np.dot(out1, WU)
        out2L = np.dot(out1, WL)
        out2 = (out2U + out2L) / 2
        outList.append(out2)
...
WL = np.random.uniform(-1, 1, (layer[1], layer[2]))
WU = np.random.uniform(-1, 1, (layer[1], layer[2]))
...
for i in range(epoch):  # Iteration over epochs    i: number of epoch
    ...
    for j in range(data_train_size):  # Iteration over rows   j: number of row
        ...
        error = (targets_data[j] - out2) / 2

        ##grad W
        gradWL = np.dot(error,out1)
        gradWU = np.dot(error,out1)
        ...
        gradC = np.dot(error,WU.T) + np.dot(error,WL.T)
        gradC = np.dot(gradC,temp)
        gradC = np.dot(out1.T,gradC)

        ## grad SIGMA
        temp = np.power(net,2) / np.power(SIGMA[0],3)
        gradSIGMA = np.dot(error,WU.T) + np.dot(error,WL.T)
    ...
    for j in range(data_test_size):  # Iteration over rows   j: number of row
        ...
        error = (targets_data[index] - out2) / 2
...
feedForwardOut = feedForward(WL,WU,C,SIGMA,dataCopy[:,:input_size],layer)
